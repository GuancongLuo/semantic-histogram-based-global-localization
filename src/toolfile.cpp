...
void insertPose(string&dir, Mat&pose, int number){
    ...
    string txtname = dir+"Trajectory_3100_7500.txt";
    ...
    {
        myfile>>h[i]>>a[i]>>b[i]>>c[i]>>d[i]>>e[i]>>f[i]>>g[i];
...
Matrix4f obtainTransformMatrix(Mat pose, int i, int type){
    ...
    if(type == 1){
        ...
        q.at<double>(0,0) = pose.at<double>(i, 4);
        q.at<double>(0,1) = pose.at<double>(i, 5);
        q.at<double>(0,2) = pose.at<double>(i, 6);
        q.at<double>(0,3) = pose.at<double>(i, 3);
        ...
        cout<<"frame_pose_obtainTransformMatrix: "<<frame_pose<<endl;
    }
    else{
...
void gatherPointCloudData(pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud, vector<vector<float> >& centerpoint, Mat pose, Mat Label, vector<uchar> label_gray, vector<float> camera, float scale, vector<std::string> dir, int endpoint, int startpoint){
    ...
    Matrix4f fake_pose = Matrix4f::Identity();
    Matrix4f fake_camera_pose = Matrix4f::Identity();

    // fake_pose(2,3) = 350;
    if(endpoint <= startpoint){
        cout<<"startpoint: "<<startpoint<<endl;
        cout<<"endpoint: "<<endpoint<<endl;
        cout<<"error input: endpoint need more than startpoint"<<endl;
    }
    ...
    for(int i =startpoint; i<endpoint; i++){
        ...
        frame_pose = obtainTransformMatrix(pose, i-startpoint, 1);
        cout<<"frame_pose: "<<frame_pose<<endl;
        
        if(i-startpoint == startpoint-startpoint){
            frame_pose0 = frame_pose;
        }
        camera_pose = frame_pose0.inverse() * frame_pose;
        fake_pose(2,3) = fake_pose(2,3) + 1;
        cout<<"fake_pose: "<<fake_pose<<endl;
        cout<<"fake_camera_pose: "<<fake_camera_pose<<endl;

        //load the rgb image, depth image and the segmentation image
        char base_name[256];
        sprintf(base_name,"%08d.png",i);
        string depth_file_name = dir[0]+"/" + base_name;
        string seg_file_name = dir[1]+"/"+base_name;
        //only SYNTHIA need
        //string label_file = dir+"label/segmentation_"+base_name;

        Mat depth = imread(depth_file_name,0);
        Mat image_rgb = imread(seg_file_name);
        //only SYNTHIA need
        //Mat lab = imread(label_file,0);

        //3D recontrcute the points
        pointCloudMapping.insertValue(cloud, depth, image_rgb, fake_pose, camera, scale, centerpoint, Label);
        ...
        // pointCloudMapping.insertValue(cloud, depth, image_rgb, fake_pose, camera, scale, centerpoint, Label);
        // pointCloudMapping.initialize3Dmap(1);
...
void gatherDenseMap(pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud, vector<vector<float> >& centerpoint, Mat pose, Mat Label, vector<float> camera, float scale, vector<std::string> dir, int endpoint, int startpoint, int type){
    //init the pointcloud mapping structure 
    pointCloudMapping pointCloudMapping;
    //initiallize the frame pose
    Matrix4f frame_pose = Matrix4f::Identity();
